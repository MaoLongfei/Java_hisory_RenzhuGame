...
public class GameFrame extends JFrame implements MouseListener, Runnable {
	...
	// 保存显示的提示信息
	String message = "BLACK goes first";
	// 保存最多拥有多少时间(秒)
	int maxTime = 0;
	// 做倒计时的线程类
	...
	public GameFrame() {
		...
		String imagePath = "" ;
		try {
			imagePath = System.getProperty("user.dir")+"/bin/img/background.jpg" ;
	...
	public void paint(Graphics g) {
		...
		// 输出标题信息
		g2.setFont(new Font("BLACK", Font.BOLD, 20));
		g2.drawString("Game Info：" + message, 50, 60);
		// 输出时间信息
		g2.setFont(new Font("BLACK", Font.BOLD, 14));
		g2.drawString("Time for BLACK:" + blackMessage, 30, 470);
		g2.drawString("Time for WHITE:" + whiteMessage, 250, 470);
	...
	public void mousePressed(MouseEvent e) {
			...
			if (x >= 10 && x <= 370 && y >= 70 && y <= 430) {
				...
				if (allChess[x][y] == 0) {
					...
					if (isBlack == true) {
						allChess[x][y] = 1;
						isBlack = false;
						message = "Turn for WHITE";
					} else {
						allChess[x][y] = 2;
						isBlack = true;
						message = "Turn for BLACK";
					}
					...
					JOptionPane.showMessageDialog(this, "There is a chess already here, please choose another place!");
				}
				this.repaint();
			}
		}
		...
		if (e.getX() >= 400 && e.getX() <= 470 && e.getY() >= 120
			...
			String input = JOptionPane
					.showInputDialog("Please input the max. time for each side (in minutes), 0 for unlimited.");
			try {
				maxTime = Integer.parseInt(input) * 60;
				if (maxTime < 0) {
					JOptionPane.showMessageDialog(this, "Negative numbers are not legal!");
				}
				if (maxTime == 0) {
				...
				// TODO Auto-generated catch block
				JOptionPane.showMessageDialog(this, "No information input.");
			}
		}
		// 点击 游戏说明 按钮
		if (e.getX() >= 400 && e.getX() <= 470 && e.getY() >= 170
			...
			JOptionPane.showMessageDialog(this,
					"This is a Renzhu game. Players alternate turns placing a chess of their color on an empty intersection. The winner is the first player to form an unbroken chain of five stones horizontally, vertically, or diagonally.");
		}
		// 点击 认输 按钮
		if (e.getX() >= 400 && e.getX() <= 470 && e.getY() >= 270
			...
			if (result == 0) {
				if (isBlack) {
					JOptionPane.showMessageDialog(this, "The BLACK gives up, the WHITE wins!");
				} else {
					JOptionPane.showMessageDialog(this, "The WHITE gives up, the BLACK wins!");
				}
				canPlay = false;
			}
		}
		...
		if (e.getX() >= 400 && e.getX() <= 470 && e.getY() >= 320
				&& e.getY() <= 350) {
			JOptionPane.showMessageDialog(this,
					"This is just a simple for practising basic programming skills.");
		}
		// 点击 退出 按钮
		...
		if (maxTime > 0) {
			while (true) {
				if (isBlack) {
					blackTime--;
					if (blackTime == 0) {
						JOptionPane.showMessageDialog(this, "No time left for the BLACK, WIHTE wins!");
					}
				} else {
					whiteTime--;
					if (whiteTime == 0) {
